    ...
    return (
      <div className="min-h-screen bg-background flex flex-col items-center justify-center px-4 pb-20">
        <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="text-center">
          ...
          <h2 className="text-xl font-display font-bold text-foreground mb-1">{t('orders.empty')}</h2>
          <p className="text-sm text-muted-foreground mb-6">{t('orders.empty_sub')}</p>
          <motion.button
            whileTap={{ scale: 0.95 }}
              ...
              >
                <div className="flex items-center justify-between mb-3">
                  ...
                  <span className="text-[11px] bg-accent text-accent-foreground px-2.5 py-0.5 rounded-full font-medium capitalize">
                    {t(`orders.status.${order.status}`, { defaultValue: order.status.replace('_', ' ') })}
                  </span>
                </div>
                <div className="flex items-center gap-2 mb-3 overflow-x-auto no-scrollbar">
                  ...
                  <span className="text-xs text-muted-foreground shrink-0 ml-1">{t('orders.items_count', { count: order.items.length })}</span>
                </div>
